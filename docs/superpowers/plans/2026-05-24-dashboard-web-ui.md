...
- **Move settings into a dedicated Settings view.** The accent-palette + show-metrics control currently lives as a compact form in the sidebar foot (posts to `/web/settings`). It should instead live behind the sidebar's **Settings** nav item, as part of making the sidebar nav functional (per-area views, `/area/{id}`-style routes). The sidebar is currently visual-only but model-ready for this. Tracked as a GitHub issue.
